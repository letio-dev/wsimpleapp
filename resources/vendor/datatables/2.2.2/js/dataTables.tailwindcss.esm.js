...
// Set defaults and custom Tailwind renderer
...
$.fn.dataTable.ext.renderer.pagingButton.tailwindcss = function (settings, buttonType, content, active, disabled) {
...
$.fn.dataTable.ext.renderer.pagingContainer.tailwindcss = function (settings, buttonEls) {
    const classes = settings.oClasses.paging;
...
$.fn.dataTable.ext.renderer.layout.tailwindcss = function (settings, container, items) {
    const row = $('<div/>', {
        class: items.full ? 'grid grid-cols-1 gap-4 mb-4' : 'grid grid-cols-2 gap-4 mb-4'
    }).appendTo(container);

    $.fn.dataTable.ext.renderer.layout._forLayoutRow(items, function (key, val) {
        let klass;
        if (val.table) {
            klass = 'col-span-2';
        } else if (key === 'start') {
