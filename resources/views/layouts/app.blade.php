...
<head>
    ...
    <!-- Styles / Scripts -->
    @vite(['resources/css/app.css', 'resources/js/app.js', 'resources/js/app.auth.js'])
    @stack('styles')
...
<body class="bg-gray-50 dark:bg-neutral-900">
                        ...
                        <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-60 bg-white shadow-md rounded-lg mt-2 dark:bg-neutral-800 dark:border dark:border-neutral-700 dark:divide-neutral-700 after:h-4 after:absolute after:-bottom-4 after:start-0 after:w-full before:h-4 before:absolute before:-top-4 before:start-0 before:w-full"
                            ...
                            <div class="py-3 px-5 bg-gray-100 rounded-t-lg dark:bg-neutral-700">
                                ...
                                <p class="text-sm font-medium text-gray-800 dark:text-neutral-200">
                                    {{ auth()->user()->name }}</p>
                                <p class="text-sm font-small text-gray-800 dark:text-neutral-200">
                                    &#64;{{ auth()->user()->username }}</p>
                            </div>
                            <div class="p-1.5 space-y-0.5">
                                <button jid="app_logout_btn"
                                    class="flex items-center w-[100%] gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300 dark:focus:bg-neutral-700 dark:focus:text-neutral-300"
                                    href="#">
        ...
        <div class="relative flex flex-col h-full max-h-full">
            ...
            <div
                ...
                <nav class="hs-accordion-group p-3 w-full flex flex-col flex-wrap" data-hs-accordion-always-open>
                    <ul id="ulMenuContent" class="flex flex-col space-y-1">
                        <li>
    ...
    <div class="w-full lg:ps-64">
        <div class="p-4 sm:p-6 space-y-4 sm:space-y-6 dark:text-neutral-200">
            @yield('content')
        </div>
    </div>
    <!-- End Content -->
...
<script>
    ...
    function renderMenu(data) {
        ...
        return data.map(item => {
            const isActive = currentUrl === item.href ? 'bg-gray-200' : '';
            const isExpanded = currentUrl === item.href ? 'true' : 'false';
            const isHidden = currentUrl === item.href ? '' : 'hidden';

            if (item.subItems) {
                const rdmString = generateRandomString(7);
                const hasActiveChild = item.subItems.some(subItem => subItem.href === currentUrl);
                const parentActiveClass = hasActiveChild ? 'bg-gray-200' : '';

                return `
                    <li class="hs-accordion" id="${item.text}_${rdmString}">
                        <a href="javascript:void()" class="hs-accordion-toggle w-full text-start flex items-center gap-x-3 py-2 px-2.5 text-sm text-gray-700 rounded-lg hover:bg-gray-200 focus:outline-none focus:bg-gray-300 ${parentActiveClass}" aria-expanded="${hasActiveChild}" aria-controls="${item.text}_${rdmString}-collapse-1">
                            ${item.icon || ''}
                            ${item.text}
                        ...
                        <div id="${item.text}_${rdmString}-collapse-1" class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300 ${hasActiveChild ? '' : 'hidden'}" role="region" aria-labelledby="${item.text}_${rdmString}">
                ...
                return `
                    <li>
                        <a class="flex items-center gap-x-3 py-2 px-2.5 text-sm text-gray-700 rounded-lg hover:bg-gray-200 ${isActive}" href="${item.href}">
                            ${item.icon || ''}
                            ${item.text}
                            ${item.badge ? `<span class="ms-auto py-0.5 px-1.5 inline-flex items-center gap-x-1.5 text-xs bg-gray-200 text-gray-800 rounded-full">${item.badge}</span>` : ''}
                        </a>
                    </li>
                `;
            }
    ...
    function generateMenu(menuData, root = true) {
        const ul = document.createElement('ul');

        if (root) {
            ul.classList.add('flex', 'flex-col', 'space-y-1');
        } else {
            ul.classList.add('ps-8', 'pt-1', 'space-y-1');
        }
        ...
        menuData.forEach(item => {
            ...
            if (item.subItems && item.subItems.length > 0) {
                // Buat accordion
                li.classList.add('hs-accordion');
                li.innerHTML = `
                    <a href="javascript:void();" class="hs-accordion-toggle w-full text-start flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700 dark:text-neutral-200">
                        ${item.icon || ''}
                        ${item.text}
                ...
                const subMenuContainer = li.querySelector('.hs-accordion-content');
                subMenuContainer.appendChild(generateMenu(item.subItems, false));
            } else {
                // Item menu biasa
                li.innerHTML = `
                    <a class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700 dark:text-neutral-200" href="${item.href}">
                        ${item.icon || ''}
                        ${item.text}
                    </a>
                `;
    ...
    const menuContainer = document.getElementById('ulMenuContent');
    menuContainer.innerHTML = "";
    menuContainer.appendChild(generateMenu(mapData));
...
<script>
    ...
    // Cek mode dari localStorage saat pertama kali load
    if (localStorage.getItem('theme') === 'dark') {
        html.classList.add('dark');
    } else {
        html.classList.remove('dark');
    }
    updateIcons(); // Update ikon sesuai mode awal
